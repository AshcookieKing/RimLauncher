...
if (store.get('pathsConfigured') === true && store.get('newbiePromptComplete') !== true) {
  ...
  playtimeTracker = createPlaytimeTracker({
    ...
    onClaimFailed: (result) => {
      ...
      if (result.error === 'not_linked') {
        ...
        body = 'На сервере не обновлён API playtime — сообщите администрации (нужен деплой launcher_api.py).';
      } else if (result.error) {
        body = `RIM POINT: ${result.error}`;
      }
