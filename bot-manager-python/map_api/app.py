...
def _sanitize_message_channel(value: str) -> str:
    ...
    s = aliases.get(s, s)
    if s in ("hint", "system_chat", "title", "introtext", "subtitle", "bis_dynamic_text", "big_center"):
        return s
    return "hint"
...
def _admin_action_payload(action: str, data: dict) -> str | None:
    ...
    if action in ("teleport", "artillery", "delete_zone", "assault_zone", "spawn_convoy", "spawn_patrol"):
        ...
        if action == "assault_zone":
            ...
            # Добавляем классы юнитов/техники если указаны
            npc_cls_raw = str(data.get("npc_class", "")).strip()
            if npc_cls_raw and re.match(r"^[A-Za-z0-9_,\s]+$", npc_cls_raw):
                payload += f"|npc_class={npc_cls_raw[:200]}"
            veh_cls_raw = str(data.get("veh_class", "")).strip()
            if veh_cls_raw and re.match(r"^[A-Za-z0-9_]+$", veh_cls_raw):
                payload += f"|veh_class={veh_cls_raw[:100]}"
            return payload[:1000]
    ...
    if action == "broadcast_message":
        ...
        if not re.match(r"^#[0-9a-fA-F]{3,8}$", color):
            color = "#ffffff"
        if ch == "big_center":
            size = max(0.5, min(float(data.get("text_size", 2.2)), 6.0))
            return f"big_center\n{color}\n{size}\n{text}"[:1000]
        return f"{ch}\n{text}"[:1000]
    if action == "play_sound_zone":
        sound_file = str(data.get("sound_file", "")).strip()
        if not re.match(r"^[A-Za-z0-9_./\\-]{3,200}$", sound_file):
            return None
        px = _clamp_coord("pos_x", data.get("pos_x", data.get("x")))
        pn = _clamp_coord("pos_y", data.get("pos_y", data.get("y")))
        radius = max(10.0, min(float(data.get("radius", 500)), 5000.0))
        volume = max(0.1, min(float(data.get("volume", 1.0)), 10.0))
        if px is None or pn is None:
            return None
        return f"{sound_file}|{px}|{pn}|{radius}|{volume}"[:1000]
    if action == "camera_view":
        px = _clamp_coord("pos_x", data.get("pos_x", data.get("x")))
        pn = _clamp_coord("pos_y", data.get("pos_y", data.get("y")))
        height = max(5.0, min(float(data.get("height", 50)), 500.0))
        radius = max(5.0, min(float(data.get("radius", 100)), 2000.0))
        duration = max(3.0, min(float(data.get("duration", 30)), 120.0))
        target = str(data.get("target_steam", "all")).strip()
        if not re.match(r"^(all|[0-9]{5,20})$", target):
            target = "all"
        if px is None or pn is None:
            return None
        return f"{px}|{pn}|{height}|{radius}|{duration}|{target}"[:1000]
    if action == "spawn_billboard":
        px = _clamp_coord("pos_x", data.get("pos_x", data.get("x")))
        pn = _clamp_coord("pos_y", data.get("pos_y", data.get("y")))
        img_raw = str(data.get("image_file", "")).strip()
        if not re.match(r"^[A-Za-z0-9_./\\-]{3,200}\.(paa|jpg|png|tga|PAA|JPG|PNG|TGA)$", img_raw):
            return None
        scale = max(0.1, min(float(data.get("scale", 1.0)), 10.0))
        direction = float(data.get("dir", 0)) % 360
        if px is None or pn is None:
            return None
        return f"{px}|{pn}|{img_raw}|{scale}|{direction}"[:1000]
...
def api_state():
    ...
    try:
        zone_spawn_queue = query_all(
            "SELECT id, template_zone_id, pos_x, pos_y, pos_z, state, error_message, created_at, "
            "trigger_radius_a, trigger_radius_b "
...
def api_admin_action():
    ...
    if not act:
        return jsonify(
            {"error": "Нужен action (kick|ban|message|lightning|grant_zeus|teleport|slay|artillery|delete_zone|assault_zone|start_quest|spawn_convoy|spawn_patrol|broadcast_message|play_sound_zone|camera_view)"}
        ), 400
    if act not in ("artillery", "delete_zone", "assault_zone", "start_quest", "spawn_convoy", "spawn_patrol", "broadcast_message", "play_sound_zone", "camera_view", "spawn_billboard") and not steam:
        return jsonify({"error": "Нужен steam_id для выбранного action"}), 400
    if act in ("artillery", "delete_zone", "assault_zone", "start_quest", "spawn_convoy", "spawn_patrol", "broadcast_message", "play_sound_zone", "camera_view", "spawn_billboard") and not steam:
        steam = "SERVER"
