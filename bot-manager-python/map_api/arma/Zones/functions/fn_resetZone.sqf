...
private _zones = missionNamespace getVariable ["SupplyZones", []]; if (_zones isEqualType []) then { _zones = _zones select { !((_x select 1) isEqualTo _trigger) }; missionNamespace setVariable ["SupplyZones", _zones, true]; };
private _meds = missionNamespace getVariable ["MedZones", []]; if (_meds isEqualType []) then { _meds = _meds select { !((_x select 1) isEqualTo _trigger) }; missionNamespace setVariable ["MedZones", _meds, true]; };
