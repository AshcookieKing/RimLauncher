...
export default function RimPointModal({ open, onClose, discordUserId, api, onSuccess }) {
  ...
  const redeem = async () => {
    ...
    setLoading(false);
    if (res.success) {
      setStatus(`Зачислено ${res.added} STAR POINT. Баланс: ${res.rim_points}`);
      setCode('');
      onSuccess?.();
    } else {
      ...
      <section className="modal-panel" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-body">
          <p className="block-hint">
            Поддержите проект на Boosty — после оплаты получите ключ подтверждения. 1 ₽ = 1 STAR POINT.
          </p>
          ...
          <label className="field">
            ...
            <input value={code} onChange={(e) => setCode(e.target.value)} placeholder="SF-XXXX-XXXX" />
          </label>
